---
title: "Practical 3: Python Loops"
...
Example:
```python
# Example: Here's a simple nested loop, looking for codons in sequences.
...
```python
# Exercise 2.2 - nested loops
# These exercises can get a bit tricky. A tip is to write the steps down in plain text before writing the actual code, to keep track of things.
# a.
# Create a nested for loop as in the example which looks for start and stop codons in the list sequences.
...
# If it's not in the list, add it. Otherwise, continue to the next bacteria. See if you can use a search engine (like google) to figure out how to append items to a list. 
---
# Solution
rev_list = []
for pat, bac_list in data.items():
  for bac in bac_list:
    if bac in rev_list:
      continue
    else:
...
for pat, bac_list in data.items():
  for bac in bac_list:
    if bac in rev_dict.keys():
      continue
    else:
      rev_dict[bac] = []
---
# e. Now let's append the patients to the lists of each key.
# The final dict should look like this: {'bacZZt98': ['pat_001', 'pat_003'], 'bac889Ytd': ['pat_001', 'pat_003'], 'bac0GFrr': ['pat_002'], 'bacFq55Hj': ['pat_003']}
# Use the dict 'data' to create your new reversed dictionary.
---
# solution
rev_dict = {}
for pat, bac_list in data.items():
  for bac in bac_list:
    if bac not in rev_dict.keys():
      rev_dict[bac] = []
    rev_dict[bac].append(pat)
---
```
